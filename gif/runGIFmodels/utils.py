...
from torch.nn import LSTM, Sequential, Linear, Sigmoid, ReLU, Conv2d, GRU
...
def get_dataset(endpath: str="split_image", skipframes: int=1, name="") -> Tuple[GIFDataset, GIFDataset, GIFDataset]:
    """
...
class RecipeImageModel(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, num_layers=1, amount_of_frames=120, backbone="vgg") -> None:
        ...
        # self.rnn = LSTM(input_size=self.input_size, hidden_size=self.hidden_size, num_layers=self.num_layers, batch_first=True)
        self.rnn = GRU(input_size=self.input_size, hidden_size=self.hidden_size, num_layers=self.num_layers, batch_first=True)
        self.relu = ReLU()
        self.fc2 = Linear(self.hidden_size, 1)
