...
class GIFDataset(Dataset):
    def __init__(
        ...
        """
        Initialize the GIFDataset object.

        :param ids: List of paths to video files.
        :param labels: List of labels corresponding to the video files.
        :param transform: Transformation to be applied to the video frames.
        :param processor: Processor to be applied to the video frames (optional).
        :param skipframes: Number of frames to skip between each processed frame.
        """
        self.ids = ids
        self.labels = labels
    ...
    def __len__(self) -> int:
        ...
        :return: Number of video files in the dataset.
        """
        return len(self.ids)

    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, int]:
        """
        Get a processed video and its corresponding label.

        :param idx: Index of the video file.
        :return frames_tr: Processed video frames.
        :return label: The corresponding label.
        """
        frames_tr = []
        v_cap = cv2.VideoCapture(self.ids[idx])
        v_len = int(v_cap.get(cv2.CAP_PROP_FRAME_COUNT))

        for fn in range(0, min(AMOUNT_OF_FRAMES, v_len)):
            ...
            if success:
                ...
                print(f"Something went wrong in {self.ids[idx]}")
        v_cap.release()

        while len(frames_tr) < AMOUNT_OF_FRAMES / self.skipframes:
            frames_tr.append(frames_tr[-1])

        assert len(frames_tr) == (AMOUNT_OF_FRAMES / self.skipframes)
...
def split_dataset(
    ...
    processor = None
    if is_hf:
        train_transformer, val_transformer = None, None
...
def get_dataset(
    endpath: str = "split_image", skipframes: int = 1, name=""
) -> Tuple[GIFDataset, GIFDataset, GIFDataset]:
    """
...
class BackBoneModelResNet(nn.Module):
    ...
    def __init__(self, resnet: torchvision.models, hidden_size: int) -> None:
        ...
        self.fc = Linear(
            2048, self.hidden_size
        )  # 2048 is the feature size for ResNet50
...
class BackBoneModelDima806(nn.Module):
    ...
    def __init__(
        ...
        self.model.classifier = torch.nn.Linear(
            in_features=768, out_features=self.input_size, bias=True
        )
        self.rnn = LSTM(
    ...
    def __init__(
        ...
        if backbone == "vgg":
            self.backbone = BackBoneModel(
                vgg16(weights="IMAGENET1K_V1"), self.input_size
            )
...
def train_and_plot(model: Any, parameters: Dict[str, Any], name: str) -> None:
    ...
    for epoch in range(num_epochs):
        ...
        plt.legend()
        plt.savefig(f"../images/trainandvalcurves_{name}.png")
...
def plot_confusionmatrix(
    ...
    plt.savefig(f"../images/confusionmatrix_{name}.png")
    plt.show()
...
def calculate_precision_recall(
    model: Any, parameters: Dict[str, Any], name: str, decision: int
) -> Tuple[int, int, int]:
    """
...
def show_measures_for_decision_boundaries(
    model: Any, parameters: Dict[str, Any], name: str
) -> None:
    """
