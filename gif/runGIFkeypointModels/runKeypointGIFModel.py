...
parser.add_argument("-is", "--inputsize", type=int, default=512)
...
name = f"keypoints_b{batch_size}s{skipframes}e{num_epochs}lr{lr}is{INPUT_SIZE}hs{HIDDEN_SIZE}nl{NUM_LAYERS}{args.name}bb{backbone}"

print("Retrieving datasets and loaders...")

train_ds, val_ds = utils.get_dataset(skipframes=args.skip, name=args.name)
...
model = utils.GIFKeypointModel(input_size=INPUT_SIZE, hidden_size=HIDDEN_SIZE, num_layers=NUM_LAYERS).to(device)
