...
class RecipeDataset(Dataset):
    ...
    def __getitem__(self, idx: List[str]) -> Tuple[torch.Tensor, str]:
        ...
        assert len(frames_tr) == (
            AMOUNT_OF_FRAMES / self.skipframes
        ), f"Expecting {AMOUNT_OF_FRAMES / self.skipframes} frames but got {len(frames_tr)} in {self.ids[idx]}"
...
) -> Tuple[RecipeDataset, RecipeDataset, RecipeDataset]:
    """
