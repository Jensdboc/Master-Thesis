...
class RecipeDataset(Dataset):
    ...
    """
    def __init__(self, ids: List[str], labels: List[str], transform, processor=None, skipframes: int=1) -> None:
        self.ids = ids
        self.labels = labels
    ...
    def __getitem__(self, idx: List[str]) -> Tuple[torch.Tensor, str]:
        """
        Return the requested item and label
        """
        frames_tr = []
        v_cap = cv2.VideoCapture(self.ids[idx])
        v_len = int(v_cap.get(cv2.CAP_PROP_FRAME_COUNT))
        
        for fn in range(0, AMOUNT_OF_FRAMES):
            ...
            if success:
                ...
                if self.processor is not None:
                    frame = self.processor(images=frame, return_tensors="pt").pixel_values
                    frame = frame.squeeze(0)
                else:
                    frame = self.transform(frame)
                if fn % self.skipframes == 0:
                    frames_tr.append(frame)
            else:
                print(f"Something went wrong in frame {fn}/{v_len} in {self.ids[idx]}")

        v_cap.release()

        assert len(frames_tr) == (AMOUNT_OF_FRAMES / self.skipframes), f"Expecting {AMOUNT_OF_FRAMES / self.skipframes} frames but got {len(frames_tr)} in {self.ids[idx]}"
...
#     Class representing the recipe dataset
#     """
#     def __init__(self, ids: List[str], labels: List[str], transform, skipframes: int=1) -> None:
#         self.ids = ids
#         self.labels = labels
#         self.skipframes = skipframes
#         self.transform = transform
...
#         assert len(frames_tr) == (AMOUNT_OF_FRAMES / self.skipframes), f"Expecting {AMOUNT_OF_FRAMES / self.skipframes} frames but got {len(frames_tr)}"
...
def split_dataset(ids: List[str], labels: List[str], groups: List[str], endpath: str="split_image", skipframes: int=1, name: str="", is_hf: bool=False) -> None:
    """
    ...
    train_ds = RecipeDataset(ids=train_ids, labels=train_labels, transform=train_transformer, skipframes=skipframes, processor=processor)
    val_ds = RecipeDataset(ids=val_ids, labels=val_labels, transform=val_transformer, skipframes=skipframes, processor=processor)
    test_ds = RecipeDataset(ids=test_ids, labels=test_labels, transform=test_transformer, skipframes=skipframes, processor=processor)
...
def get_dataset(endpath: str="split_image", skipframes: int=1, name="") -> Tuple[RecipeDataset, RecipeDataset, RecipeDataset]:
    """
...
def train_and_plot(model: Any, parameters: Dict[str, Any], name: str) -> None:
    ...
    for epoch in range(num_epochs):
        ...
        plt.legend()
        plt.savefig(f"/project_ghent/Master-Thesis/ownModelNotebooks/images/trainandvalcurves_{name}.png")
...
def plot_confusionmatrix(model: Any, parameters: Dict[str, Any], name :str, mode: str="Validation", verbose: bool=False) -> None:
    ...
    plt.savefig(f"/project_ghent/Master-Thesis/ownModelNotebooks/images/confusionmatrix_{name}.png")
    plt.show()
